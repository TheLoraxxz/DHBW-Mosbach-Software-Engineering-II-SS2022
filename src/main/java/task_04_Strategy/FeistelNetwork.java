...
public class FeistelNetwork {
    ...
    public String iterate(String input, String[] subKeys) {
        ...
        while (iterationCount < 16) {
            rightValue = xor(rightValue, fFunction(leftValue, subKeys[iterationCount++]));
        }
    ...
    public String xor(String string01, String string02) {
        ...
        for (int index = 0; index < string01Bits.length; index++) {
            if (string01Bits[index] != string02Bits[index]) {
                resultBits[index] = '1';
            } else {
                resultBits[index] = '0';
            }
    ...
    public String expansion(String string) {
        char[] expandedBits = new char[48];
        char[] rBits = string.toCharArray();

        for (int index = 0; index < 48; index++) {
            expandedBits[index] = rBits[expansionMatrix[index / 6][index % 6] - 1];
        }
        return new String(expandedBits);
    }

    public String applySBox(String string) {
        ...
        for (int index = 0; index < 48; index += 6) {
            sixBits = string.substring(index, index + 6);
    ...
    public String applyPermutation(String string) {
        char[] result = new char[32];

        for (int index = 0; index < 32; index++) {
            result[index] = string.charAt(permutationMatrix[index / 8][index % 8] - 1);
